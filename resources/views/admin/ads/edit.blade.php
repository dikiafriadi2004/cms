...
@section('page-title', 'Edit Ad: ' . $ad->name)
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="adsense" {{ old('type', $ad->type) == 'adsense' ? 'selected' : '' }}>Google AdSense</option>
                        <option value="adsera" {{ old('type', $ad->type) == 'adsera' ? 'selected' : '' }}>Adsera</option>
                        <option value="manual" {{ old('type', $ad->type) == 'manual' ? 'selected' : '' }}>Manual HTML</option>
                    </select>
                    @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="position" id="position" required
                        ...
                        <option value="in_content" {{ old('position', $ad->position) == 'in_content' ? 'selected' : '' }}>In Content (Di Tengah Artikel)</option>
                        <option value="between_posts" {{ old('position', $ad->position) == 'between_posts' ? 'selected' : '' }}>Between Posts</option>
                    </select>
                    @error('position')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div id="inContentParagraphField" style="display: none;">
                    ...
                    <input type="number" name="in_content_paragraph" id="in_content_paragraph" value="{{ old('in_content_paragraph', $ad->in_content_paragraph ?? 3) }}" min="1"
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $ad->sort_order) }}" min="0"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="rotation_group" id="rotation_group" value="{{ old('rotation_group', $ad->rotation_group) }}"
                ...
                <div>
                    ...
                    <select name="rotation_mode" id="rotation_mode"
                        ...
                        <option value="random" {{ old('rotation_mode', $ad->rotation_mode) == 'random' ? 'selected' : '' }}>Random - Show random ad from group</option>
                        <option value="weighted" {{ old('rotation_mode', $ad->rotation_mode) == 'weighted' ? 'selected' : '' }}>Weighted - Based on weight value</option>
                        <option value="sequential" {{ old('rotation_mode', $ad->rotation_mode) == 'sequential' ? 'selected' : '' }}>Sequential - Rotate evenly based on impressions</option>
                    </select>
                ...
                <!-- Rotation Weight -->
                <div id="rotationWeightField" style="{{ old('rotation_mode', $ad->rotation_mode) == 'weighted' ? '' : 'display: none;' }}">
                    <label for="rotation_weight" class="block text-sm font-medium text-gray-700 mb-1">
                        Rotation Weight
                    </label>
                    <input type="number" name="rotation_weight" id="rotation_weight" value="{{ old('rotation_weight', $ad->rotation_weight) }}" min="1" max="100"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $ad->start_date?->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $ad->end_date?->format('Y-m-d')) }}"
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-1">
                        {{ $ad->image ? 'Replace Image' : 'Upload Image' }} {{ !$ad->image ? '*' : '' }}
                    </label>
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="open_new_tab" value="1" {{ old('open_new_tab', $ad->open_new_tab) ? 'checked' : '' }}
                ...
                <div class="border-t pt-4 mt-4">
                    ...
                    <select name="size_preset" id="size_preset"
                        ...
                        <option value="medium" {{ old('size_preset', $ad->size_preset ?? 'medium') == 'medium' ? 'selected' : '' }}>Medium (Rekomendasi)</option>
                        <option value="small" {{ old('size_preset', $ad->size_preset) == 'small' ? 'selected' : '' }}>Small (Kecil)</option>
                        <option value="large" {{ old('size_preset', $ad->size_preset) == 'large' ? 'selected' : '' }}>Large (Besar)</option>
                        <option value="auto" {{ old('size_preset', $ad->size_preset) == 'auto' ? 'selected' : '' }}>Auto (Full Width Responsive)</option>
                        <option value="custom" {{ old('size_preset', $ad->size_preset) == 'custom' ? 'selected' : '' }}>Custom (Atur Sendiri)</option>
                    </select>
                    ...
                    <div id="customSizeFields" style="display: none;" class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="custom_width" id="custom_width" value="{{ old('custom_width', $ad->custom_width) }}" min="50" max="2000"
                                placeholder="e.g., 728"
                        ...
                        <div>
                            ...
                            <input type="number" name="custom_height" id="custom_height" value="{{ old('custom_height', $ad->custom_height) }}" min="50" max="2000"
                                placeholder="e.g., 90"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                    </div>
        ...
        <div id="codeSection" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div>
                ...
                <textarea name="code" id="code" rows="10"
                    ...
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono text-sm @error('code') border-red-500 @enderror">{{ old('code', $ad->code) }}</textarea>
                    ...
                    <div class="space-y-2">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="display_rules[pages][]" value="blog_detail" 
                                {{ in_array('blog_detail', $displayPages) ? 'checked' : '' }}
                                class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="display_rules[pages][]" value="static_page" 
                                {{ in_array('static_page', $displayPages) ? 'checked' : '' }}
                                class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleSections() {
        ...
        if (selectedType === 'image') {
            ...
            // Only require image if no current image exists or if removing current image
            if (!currentImagePreview || (removeImageCheckbox && removeImageCheckbox.checked)) {
    ...
    toggleRotationWeightField(); // Initial state
});
</script>
@endsection
