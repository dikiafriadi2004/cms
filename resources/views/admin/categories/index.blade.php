...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-500">{{ $category->posts_count ?? 0 }} posts</td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <button type="button" onclick="deleteCategory({{ $category->id }}, '{{ route('admin.categories.destroy', $category) }}')" class="text-red-600 hover:text-red-900 transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
@push('scripts')
<script>
async function deleteCategory(categoryId, deleteUrl) {
    const confirmed = await window.confirmDelete('Delete Category', 'Are you sure you want to delete this category? This action cannot be undone.');
    if (!confirmed) return;
    ...
    try {
        ...
        if (data.success) {
            window.showToast('success', data.message || 'Kategori berhasil dihapus!');
