...
<body class="antialiased bg-white">
                ...
                @if($headerMenu && $headerMenu->items)
                    @foreach($headerMenu->items as $item)
                        @if($item->is_active)
                            @if($item->children->count() > 0)
                                <div class="space-y-1">
                                    <button class="w-full text-left text-gray-700 hover:bg-purple-50 hover:text-purple-600 px-3 py-2 text-base font-medium rounded-md">{{ $item->title }}</button>
                                    @foreach($item->children as $child)
                                        @if($child->is_active)
                                            <a href="{{ $child->url }}" target="{{ $child->target }}" class="block pl-6 pr-3 py-2 text-sm text-gray-600 hover:bg-purple-50 hover:text-purple-600 rounded-md">{{ $child->title }}</a>
                                        @endif
                                    @endforeach
                                </div>
                            @else
                    ...
                    @endforeach
                @else
                    <a href="{{ route('home') }}" class="block text-gray-700 hover:bg-purple-50 hover:text-purple-600 px-3 py-2 text-base font-medium rounded-md">Home</a>
                    <a href="{{ route('blog.index') }}" class="block text-gray-700 hover:bg-purple-50 hover:text-purple-600 px-3 py-2 text-base font-medium rounded-md">Blog</a>
                @endif
    ...
    @php $headerAds = \App\Models\Ad::getByPosition('header', ['page' => request()->route()->getName()]); @endphp
    @if($headerAds->count() > 0)
        <div class="pt-16">
            @foreach($headerAds as $ad)
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">{!! $ad->code !!}</div>
            @endforeach
        </div>
    @endif
            ...
            <div>
                ...
                <ul class="space-y-2 text-gray-400">
                    ...
                    @if(isset($settings['contact_address']) && $settings['contact_address'])
                        <li class="flex items-start"><svg class="w-5 h-5 mr-2 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg><span>{{ $settings['contact_address'] }}</span></li>
                    @endif
                </ul>
            </div>
            <div>
...
@php $footerAds = \App\Models\Ad::getByPosition('footer', ['page' => request()->route()->getName()]); @endphp
