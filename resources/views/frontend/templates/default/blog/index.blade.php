...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 bg-white">
    ...
    <div class="flex flex-wrap items-center justify-center gap-3 mb-16">
        <a href="{{ route('blog.index') }}" class="px-6 py-2.5 rounded-xl {{ !request()->route('category') ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20' : 'bg-white text-slate-600 border border-slate-200 hover:border-brand-600 hover:text-brand-600' }} font-bold text-sm transition">
            Semua
        </a>
        @foreach($categories as $category)
        <a href="{{ route('blog.category', $category->slug) }}" class="px-6 py-2.5 rounded-xl {{ request()->route('category') == $category->slug ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20' : 'bg-white text-slate-600 border border-slate-200 hover:border-brand-600 hover:text-brand-600' }} font-bold text-sm transition">
            {{ $category->name }}
        </a>
        @endforeach
    </div>
