...
@section('content')
    <div class="content">
                    ...
                    <tbody>
                        @foreach ($pages as $page)
                            <tr class="intro-x">
                                ...
                                <td>
                                    <div class="text-slate-500 font-medium whitespace-nowrap mt-0.5">{{ $page->title }}
                                    </div>
                                </td>
                                <td>
                                    <div class="text-slate-500 font-medium whitespace-nowrap mt-0.5">{{ $page->slug }}
                                    </div>
                                </td>
                                <td class="text-center">{{ $page->created_at->isoFormat('dddd, D MMMM Y') }}</td>
                                <td class="w-40">
                                    @if ($page->status == 'Published')
                                        <div class="flex items-center text-success"> <i data-lucide="check-square"
                                                class="w-4 h-4 mr-2"></i> {{ $page->status }}
                                        </div>
                                    @else
                                ...
                                <td class="table-report__action w-56">
                                    <div class="flex justify-center items-center">
                                        ...
                                        <button class="flex items-center  text-danger" data-tw-toggle="modal"
                                            ...
                                            data-tw-id="{{ $page->id }}"> <i data-lucide="trash-2"
        ...
        @include('backend.admin.pages.modal-delete')
    </div>
@endsection
